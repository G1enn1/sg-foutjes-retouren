...
class STM_DB {
	...
	public static function insert( array $row ) {
		...
		$data = array(
			...
			'thread_id'        => ( isset( $row['thread_id'] ) && '' !== $row['thread_id'] ) ? $row['thread_id'] : null,
			'source'           => $row['source'],
		...
		// Upsert on dedup_key: duplicates no-op, except that a re-sync may
		// backfill thread_id on rows stored before threads were captured.
		$columns      = implode( ', ', array_map( array( __CLASS__, 'ident' ), array_keys( $data ) ) );
		$placeholders = implode( ', ', array_fill( 0, count( $data ), '%s' ) );
		$sql          = "INSERT INTO {$table} ({$columns}) VALUES ({$placeholders})
			ON DUPLICATE KEY UPDATE thread_id = COALESCE(VALUES(thread_id), thread_id)";
		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		$prepared = $wpdb->prepare( $sql, array_values( $data ) );
		$wpdb->query( $prepared );

		// rows_affected: 1 = inserted, 2 = updated existing, 0 = unchanged.
		return ( 1 === (int) $wpdb->rows_affected ) ? 1 : 0;
	}
